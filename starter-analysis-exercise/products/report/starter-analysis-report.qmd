...
Data was pulled in for all weeks of the season with the CFBFastR package. Some values were pulled in as the wrong datatype, so key data was converted to the correct data type. The weekly box scores were then aggregated to include averages at each week in the season. For full code, refer to the statistical-analysis.R 

## Statistical analysis
_Explain anything related to your statistical analyses._
Multiple models were created to attempt to predict the outcome of games. A linear model was used to predict score and then win was given to the team with the higher predicted score. 
